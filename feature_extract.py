...
df = pd.read_csv("./data/ielts_data.csv") 
...
for idx, row in df.iterrows():
    ...
    embeddings = sbert_model.encode([prompt, essay])
    sbert_cos_sim = cosine_similarity([embeddings[0]], [embeddings[1]])[0][0]
    ...
    features.append([
        ...
        tfidf_cos_sim, sbert_cos_sim,
        mean_tfidf_score, std_tfidf_score, unique_word_ratio,
        window_repeat_count, grammar_errors,
        row['score']
    ])
...
feature_df = pd.DataFrame(features, columns=[
    ...
    'tfidf_cosine_similarity', 'sbert_prompt_adherence_similarity',
    'mean_tfidf_score', 'std_tfidf_score', 'unique_word_ratio',
    'window_repeat_count', 'num_grammatical_errors',
    'score'
])
